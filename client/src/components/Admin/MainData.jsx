...
import Web3Chart from './Web3Chart';
...
const MainData = () => {
    ...
    const totalAmount = safeOrders.reduce((total, order) => total + order.totalPrice, 0);
        ...
        datasets: [
            {
                ...
                data: months.map((m, i) => safeOrders.filter((od) => new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear() - 2).reduce((total, od) => total + od.totalPrice, 0)),
            },
            {
                label: `Sales in ${date.getFullYear() - 1}`,
                borderColor: 'orange',
                backgroundColor: 'orange',
                data: months.map((m, i) => safeOrders.filter((od) => new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear() - 1).reduce((total, od) => total + od.totalPrice, 0)),
            },
            {
                label: `Sales in ${date.getFullYear()}`,
                borderColor: '#4ade80',
                backgroundColor: '#4ade80',
                data: months.map((m, i) => safeOrders.filter((od) => new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear()).reduce((total, od) => total + od.totalPrice, 0)),
            },
        ],
    };
        ...
        datasets: [
            {
                ...
                data: statuses.map((status) => safeOrders.filter((item) => item.orderStatus === status).length),
            },
        ],
    };
        ...
        datasets: [
            {
                ...
                data: [outOfStock, safeProducts.length - outOfStock],
            },
        ],
    };
        ...
        datasets: [
            {
                ...
                data: categories.map((cat) => safeProducts.filter((item) => item.category === cat).length),
            },
        ],
    };
    ...
    return (
        <>
            <MetaData title="Admin Dashboard | AtEMkart" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-6">
                ...
                <div className="flex flex-col bg-red-500 text-white gap-2 rounded-xl shadow-lg hover:shadow-xl p-6">
                    ...
                    <h2 className="text-2xl font-bold">{safeOrders.length}</h2>
                </div>
                <div className="flex flex-col bg-yellow-500 text-white gap-2 rounded-xl shadow-lg hover:shadow-xl p-6">
                    ...
                    <h2 className="text-2xl font-bold">{safeProducts.length}</h2>
                </div>
                <div className="flex flex-col bg-green-500 text-white gap-2 rounded-xl shadow-lg hover:shadow-xl p-6">
                    ...
                    <h2 className="text-2xl font-bold">{safeUsers.length}</h2>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-8 min-w-full mt-6">
